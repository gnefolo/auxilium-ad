...
**Limite importante**: il database è SQLite su disco locale del servizio. Sul piano gratuito di Render il disco **non è persistente tra i deploy** (viene ricreato da zero a ogni nuovo deploy). I dati "reali" (bilanci, Elenco Servizi, tavole ISTAT) si riseminano automaticamente all'avvio — nessun problema. Ma i dati inseriti a mano nella dashboard **andrebbero persi ad ogni nuovo deploy**: questo vale per i progetti "Calcolo SROI" e i valori di outcome nella pagina SROI (come già notato), e da questa fase vale anche per le **Gare d'Appalto** (`/api/tenders`) e per le voci di bilancio inserite manualmente (`/api/finance/entries`).

Valutato migrare a Postgres persistente in questa fase: il piano gratuito di Render Postgres **scade dopo 30 giorni** (14 giorni di grazia prima della cancellazione se non si passa a un piano pagato) — non risolve il problema, semplicemente lo sposta più in là. Scelta per ora: restare su SQLite e accettare il limite. Se in futuro si vuole risolvere davvero, le opzioni sono un disco persistente Render (a pagamento), Render Postgres a pagamento (~6-7$/mese), o Supabase Postgres gratuito (nessun costo, ma il progetto si sospende dopo 7 giorni di inattività — si riattiva senza perdita dati, ma se resta sospeso troppo a lungo rischia la cancellazione).
